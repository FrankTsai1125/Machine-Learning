...
def sce(
    ...
    if density < 1.0:
        ...
        variance_mask = sce_mask(task_tensors, density)
        # Expand mask to match task_tensors shape [num_tasks, param_shape...]
        variance_mask = variance_mask.unsqueeze(0).expand_as(task_tensors)
        # Apply the binary mask across all task tensors.
        task_tensors = task_tensors * variance_mask
    ...
    masked_task_tensors = task_tensors * majority_sign_mask
    weighted_task_tensors = masked_task_tensors * weights
    ...
    weight_sum = (weights * majority_sign_mask).sum(dim=0)
    mixed_task_tensors = mixed_task_tensors / torch.clamp(weight_sum, min=1e-6)
